...
public class Painter : MonoBehaviour
{
    ...
    private bool hasUserPaintedSinceReset;
    ...
    public event Action<bool> OnCanvasDirtyChanged;

    public Texture2D CanvasTexture => canvasTexture;
    public bool HasUserPaintedSinceReset => hasUserPaintedSinceReset;
    ...
    public void LoadCanvasFromTexture(Texture2D sourceTexture, bool markDirty = false)
    {
        if (canvasTexture == null || sourceTexture == null)
            return;

        Texture2D readableSource = GetReadableTexture(sourceTexture);
        Color[] resizedPixels = ScalePixels(readableSource, textureWidth, textureHeight);
        canvasTexture.SetPixels(resizedPixels);
        canvasTexture.Apply();
        SetCanvasDirty(markDirty);
        EnsureCanvasAssigned();
    }

    public byte[] EncodeCanvasToPng()
    {
        return canvasTexture != null ? canvasTexture.EncodeToPNG() : null;
    }

    public void MarkCanvasAsClean()
    {
        SetCanvasDirty(false);
    }
    ...
    private void SetCanvasDirty(bool isDirty)
    {
        if (hasUserPaintedSinceReset == isDirty)
            return;

        hasUserPaintedSinceReset = isDirty;
        OnCanvasDirtyChanged?.Invoke(hasUserPaintedSinceReset);
    }

    private static Color[] ScalePixels(Texture2D source, int targetWidth, int targetHeight)
    {
        Color[] pixels = new Color[targetWidth * targetHeight];

        for (int y = 0; y < targetHeight; y++)
        {
            float v = targetHeight <= 1 ? 0.5f : y / (float)(targetHeight - 1);
            for (int x = 0; x < targetWidth; x++)
            {
                float u = targetWidth <= 1 ? 0.5f : x / (float)(targetWidth - 1);
